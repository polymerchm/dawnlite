...
def create_app():
    ...
    app.config['REDIS_ALARM_QUEUE_KEY'] = 'dawnlite_alarm_queue'
    app.config['REDIS_STATE_KEY'] = 'dawnlite_state'
   
    app.config['REDIS_REMOTE_QUEUE_KEY'] = 'dawnlite_remote_queue'
    ...
    app.config['RAMP_DURATION'] = 10 # seconds  
    app.config['PWM_FREQUENCY'] = 25_000 # 25 KHz 
...
def patch_light():
    ...
    if new_light_on != state.light_on:
        comm.send_message(app, comm.SetLightStateMessage(on=new_light_on), alarm_queue)
